...
function getContentType(filePath) {
  ...
  return extension === '.css' ? 'text/css; charset=utf-8' : extension === '.js' ? 'text/javascript; charset=utf-8' : 'text/html; charset=utf-8';
}
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/partners') {
        const partner = service.createPartner(await readJson(request));

        sendJson(response, 201, partner);
        return;
      }
...
if (require.main === module) {
  ...
  application.server.listen(3002, () => {
    console.log('Откройте http://localhost:3002');
  });
